...
async def root():
    return {
        ...
        "endpoints": [
            ...
            "/matches - Get all upcoming matches",
            "/matches/range - Get matches across a date range"
        ]
    }
...
async def get_match(match_id: str):
    ...
    if is_upcoming:
        ...
        # Method 1: roster sections on match page
        roster_sections = soup.select(".match-streams-container")

        # Method 2: player names in header
        ...
        # Method 3: fetch rosters by team ID if missing
...
async def get_team(team_id: str):
    ...
    recent_matches = []
    match_items = soup.select(".mod-dark .wf-card")[:5]
...
async def get_player(player_id: str):
    ...
    for item in stat_items:
        ...
        if label_el and value_el:
            ...
            stats[label] = value
    recent_matches = []
    match_rows = soup.select(".mod-dark tr.wf-card")[:10]
...
# ============== EXTENDED MATCHES RANGE ENDPOINT ==============

@app.get("/matches/range")
async def get_matches_range(days_ahead: int = 180, days_back: int = 180):
    """
    Scrape matches across a date range to include far future/past days.
    Returns { matches: [ {id, team1, team2, event, start_time} ] }
    """
    today = datetime.utcnow().date()
    start_date = today - timedelta(days=abs(days_back))
    end_date = today + timedelta(days=abs(days_ahead))
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        ...
        while current <= end_date:
            url = f"https://www.vlr.gg/matches?date={current.isoformat()}"
            try:
            ...
            for item in match_items:
                ...
                time_el = item.select_one(".match-item-time")
                ts = time_el.get("data-utc-ts", "") if time_el else ""
                start_time = ""
                try:
                    start_time = datetime.utcfromtimestamp(int(ts)).isoformat() if ts else current.isoformat()
                except Exception:
